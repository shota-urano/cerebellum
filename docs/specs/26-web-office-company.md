...
6. **全景に部署の導線を増やさない**（20 §3.1-1 の4部屋＋MY DESK のまま。入口は社員カードの所属部署タップと会社案内シートだけ）（**→ [27](./27-web-office-departments.md) で取り消し。全景の部屋そのものが部署になる**）
...
2. 部署の並びは **`employees` の返却順で最初に現れた `dept` の順**（**[27](./27-web-office-departments.md) §3.3-1 以降は `office.json` の `departments[].order` があればその順。本規則は `departments` が無いときのフォールバック**）。cerebellum 側に部署の順序表を持たない。`dept` が `null` の社員は末尾に「部署 未記載」としてまとめる（隠さない）
...
6. 全景（`/office`）からの導線は**1つだけ**（ヘッダの「会社案内」リンク）。全景の4部屋＋MY DESK の構図を変えない（**→ [27](./27-web-office-departments.md) で取り消し。全景は8部署＋MY DESK になる。会社案内リンクは残る**）
...
- ~~20 §3.1-3 の部屋分類規則（4部屋）の変更。部署（`dept`）で部屋を切り直さない——部屋は表示上の役割グルーピング、部署は組織図の所属で、別軸として並存させる（ライン絞り込みと同じ整理）~~ → **[27](./27-web-office-departments.md) で取り消し**。2026-09-03、画面上で並存が伝わらず「部署が古い。4つしか無い」と読まれたため、部屋を `dept` で切り直す判断に変えた（`line` は上下流の軸として残す）
