...
1. `detailRef` を持つタスク行の**右端に詳細シェブロン（›）**を出す。これは行本体とは**別のタップ領域**にする
2. 行本体のタップは**従来どおりチェックのトグル**（挙動を変えない）。シェブロンのタップだけが `/digest?date={その日}&section={detailRef の後半}` へ遷移する
3. シェブロンのタップターゲットは 44px 以上を確保し、誤タップでトグルが走らないようにする（`stopPropagation`）
4. `detailRef` が無いタスク行の見た目は現状のまま（シェブロンを出さない）
