...
- **md 描画（`shared/ui/Markdown`）の対応記法**（`lessonMd` / `questionMd` / `answerMd`（[`14`](./14-learning.md) §3.1）・`detailMd`（[`17`](./17-harness-approval.md)）の共通描画。各画面はここを参照し、記法を二重定義しない）:
  ブロックは**見出し・箇条書き・番号付き・引用・コードブロック・パイプテーブル**、インラインは `` `code` ``・`**強調**`・`[text](url)`。`[text](url)` のうち `http(s)://` で始まる外部リンクは `target="_blank" rel="noopener noreferrer"` で新しいタブに開く（画面は「読む場所」で、記事へ飛んで戻れなくならないため。2026-09-14）。相対パス・`#` は同じタブ。外部ライブラリは入れず自前パーサで、**未対応の記法は素のテキストとして出す**（壊れても読める側に倒す。`dangerouslySetInnerHTML` は使わない）
