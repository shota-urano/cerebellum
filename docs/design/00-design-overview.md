...
- 今日画面 = specs 08、履歴画面 = specs 09、シェル = specs 07 に対応。**specs に無い画面は無い**
...
- エラーバナー文言は「Vault が読み取れません。同期完了後に自動で再試行します」で**確定**（specs 07 §6 の「Vault 同期中系」を具体化）
