...
/** runs は新しい順（§2）。outcome の判定は生成側の責務で、画面は値で塗るだけ（§3.4） */
const RUNS = [
  {
    run_id: 'r-collect',
    automation_id: 'a-collect',
...
test('勤務帯が返却順（01:00→22:00）で並び、勤務ラベル・名前・直近 run の状態と headline が出る', async ({
  ...
  await expect(rows.nth(0)).toContainText('夜勤（night-shift）');
  await expect(rows.nth(0)).toContainText('cerebellum で4件クローズしました。');
  await expect(rows.nth(1)).toContainText('ハーネス取り込み判定（night-harness）');
  await expect(rows.nth(1)).toContainText('取り込み候補を2件出しました。');
  await expect(rows.nth(4)).toContainText('新しいつながりは見つかりませんでした。');
