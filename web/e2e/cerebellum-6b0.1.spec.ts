...
// 外部の脳ビュー「brain」と常設の「オフィス」が加わり7項目、
// docs/specs/23-web-waiting.md の実装で「あなた待ち」が加わり8項目（docs/specs/16 §3.3）。
...
test('ハンバーガーでドロワーが開き、バックドロップタップで閉じる', async ({ page }) => {
  ...
  // 8項目が頻度順に並んでいる（docs/specs/16 §3.3）
...
test('ドロワーの7項目それぞれへ遷移し、遷移先でアクティブ表示になる', async ({ page }) => {
