...
export default defineConfig({
  ...
  use: {
    ...
    // viewport を録画サイズと一致させる（iPhone 相当・このアプリはモバイル前提）。
    // viewport 未指定だと既定 1280×720（横長）で描画され、縦長の録画枠に縮小されて
    // 上下黒帯＋小さい横長画面になる——スマホで見られない動画になる（2026-07-29 実測）
    viewport: { width: 390, height: 844 },
    video: { mode: 'on', size: { width: 390, height: 844 } },
    // 録画を人間の操作速度に近づける（既定速度だと動画が速すぎて検証内容を目で追えない
    // ——2026-07-29 ユーザー指示）。E2E は数十秒遅くなるが、録画は人間が見る素材なので
    // 見られる速度であることが機能要件。検証の実質は変わらない
