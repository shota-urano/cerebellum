...
use crate::{
    ...
    usecase::{
        ...
        ports::{
            Clock, DigestRepository, HarnessRepository, InboxRepository, IntakeRepository,
            LearningRepository, RoutineRepository, TaskRepository,
        },
...
fn test_app_with_routines(seed_routines: bool) -> axum::Router {
    ...
    let inbox_repository: Arc<dyn InboxRepository> = repository.clone();
    ...
    let manage_inbox = Arc::new(ManageInbox::new(inbox_repository, Arc::clone(&clock)));
...
async fn import_result_lane(app: axum::Router, lane: &str, no: u32) {
    let mut payload = lane_set_payload(lane, lane);
    payload["problems"] = json!([{ "no": no, "questionMd": "q", "answerMd": "a" }]);
    assert_eq!(
        call_json(app, "POST", "/api/learning/sets", payload)
            .await
            .status(),
        StatusCode::OK
    );
...
#[tokio::test]
async fn learning_result_matches_problem_numbers_only_in_its_lane() {
    let app = test_app();
    import_result_lane(app.clone(), "main", 1).await;
    import_result_lane(app.clone(), "en", 2).await;
    for (lane, own, other) in [("main", 1, 2), ("en", 2, 1)] {
        let uri = format!("/api/learning/sets/today/result?lane={lane}");
        let valid = json!({"grades":[{"no":own,"grade":"o"}],"feeling":""});
        let response = call_json(app.clone(), "POST", &uri, valid).await;
        assert_eq!(response.status(), StatusCode::OK);
        ...
        let response = call_json(
            ...
            "POST",
            &uri,
            json!({"grades":[{"no":other,"grade":"o"}],"feeling":"invalid"}),
        )
        .await;
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        assert_eq!(json_body(response).await["error"]["code"], "bad_request");
        assert_eq!(json_body(call(app.clone(), "GET", &uri).await).await, saved);
    }
}

#[tokio::test]
async fn learning_results_upsert_independently_and_default_to_main() {
    let app = test_app();
    for lane in ["main", "en"] {
        import_result_lane(app.clone(), lane, 1).await;
    }
    let mut expected = std::collections::HashMap::new();
    for (lane, grade, feeling) in [
        ("main", "o", "main first"),
        ("en", "x", "en first"),
        ("main", "d", "main replaced"),
        ("en", "o", "en replaced"),
    ] {
        let uri = format!("/api/learning/sets/today/result?lane={lane}");
        let response = call_json(
            ...
            "POST",
            &uri,
            json!({"grades":[{"no":1,"grade":grade,"answer":feeling}],"feeling":feeling}),
        )
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        let saved = json_body(response).await;
        assert_eq!(saved["feeling"], feeling);
        assert_eq!(
            saved["grades"],
            json!([{"no":1,"grade":grade,"answer":feeling}])
        );
        expected.insert(lane, saved);
        for (read_lane, saved) in &expected {
            let response = call(
                app.clone(),
                "GET",
                &format!("/api/learning/sets/2026-07-25/result?lane={read_lane}"),
            )
            .await;
            assert_eq!(response.status(), StatusCode::OK);
        ...
        assert_eq!(
            json_body(call(app.clone(), "GET", "/api/learning/sets/today/result").await).await,
            expected["main"]
        );
    }
    let response = call_json(
        ...
        json!({"grades":[],"feeling":"default main"}),
    )
    .await;
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(
        json_body(
            call(
                app.clone(),
                "GET",
                "/api/learning/sets/today/result?lane=main"
            )
            .await
        )
        .await["feeling"],
    ...
    assert_eq!(
        json_body(call(app, "GET", "/api/learning/sets/today/result?lane=en").await).await,
        expected["en"]
    );
}

#[tokio::test]
async fn learning_result_missing_lane_is_not_found_independently() {
    for (present, missing) in [("main", "en"), ("en", "main")] {
        let app = test_app();
        import_result_lane(app.clone(), present, 1).await;
        let response = call_json(
            app.clone(),
            "POST",
            &format!("/api/learning/sets/today/result?lane={present}"),
            json!({"grades":[],"feeling":"recorded"}),
        )
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        let uri = format!("/api/learning/sets/today/result?lane={missing}");
        let response =
            call_json(app.clone(), "POST", &uri, json!({"grades":[],"feeling":""})).await;
        assert_eq!(response.status(), StatusCode::NOT_FOUND);
        assert_eq!(json_body(response).await["error"]["code"], "not_found");
        for has_set in [false, true] {
            if has_set {
                import_result_lane(app.clone(), missing, 1).await;
            }
            let response = call(app.clone(), "GET", &uri).await;
            assert_eq!(response.status(), StatusCode::NOT_FOUND);
            assert_eq!(json_body(response).await["error"]["code"], "not_found");
...
#[tokio::test]
async fn learning_result_validates_date_then_lane_before_reading_body() {
    let app = test_app();
    let baseline =
        json_body(call(app.clone(), "GET", "/api/learning/sets/tomorrow/result").await).await;
    for (lane, encoded) in [
        ("english", "english"),
        ("", ""),
        ("MAIN", "MAIN"),
        ...
        for date in ["today", "tomorrow"] {
            let uri = format!("/api/learning/sets/{date}/result?lane={encoded}");
            let expected = if date == "tomorrow" {
                baseline.clone()
            } else {
                json!({"error":{"code":"bad_request","message":format!("unknown lane: {lane}")}})
            };
            let response = call(app.clone(), "GET", &uri).await;
            assert_eq!(response.status(), StatusCode::BAD_REQUEST);
            assert_eq!(json_body(response).await, expected);
            for body in ["{", "{}", r#"{"grades":[],"feeling":""}"#] {
                let response = call_json_body(app.clone(), "POST", &uri, body.to_owned()).await;
                assert_eq!(response.status(), StatusCode::BAD_REQUEST);
                assert_eq!(json_body(response).await, expected);
            }
        }
...
fn inbox_batch(source: &str, date: &str, items: Value) -> Value {
    json!({"source":source,"date":date,"items":items})
}

#[tokio::test]
async fn inbox_http_round_trip_supports_approve_and_choose() {
    let app = test_app();
    let empty_items = call(app.clone(), "GET", "/api/inbox/items?status=open").await;
    assert_eq!(empty_items.status(), StatusCode::OK);
    assert_eq!(json_body(empty_items).await, json!({"items":[]}));
    let empty_summary = call(app.clone(), "GET", "/api/inbox/summary").await;
    assert_eq!(empty_summary.status(), StatusCode::OK);
    assert_eq!(json_body(empty_summary).await, json!({"sources":[]}));

    let newer = call_json(
        app.clone(), "POST", "/api/inbox/batches",
        inbox_batch("sender", "2026-07-26", json!([
            {"slug":"choose","kind":"choose","title":"newer choose","options":[{"id":"one","label":"One"},{"id":"two","label":"Two"}],"payload":{"opaque":true}}
        ])),
    ).await;
    assert_eq!(newer.status(), StatusCode::OK);
    assert_eq!(json_body(newer).await["itemCount"], 1);
    let older = call_json(
        app.clone(),
        "POST",
        "/api/inbox/batches",
        inbox_batch(
            ...
            "2026-07-25",
            json!([
                {"slug":"approve","kind":"approve","title":"older approve","bodyMd":"details"}
            ]),
        ),
    )
    .await;
    ...
    let open = json_body(call(app.clone(), "GET", "/api/inbox/items?status=open").await).await;
    assert_eq!(
        open["items"][0]["slug"], "choose",
        "open list is newest first"
    );
    assert_eq!(open["items"][1]["slug"], "approve");
    let choose_id = open["items"][0]["id"].as_i64().unwrap();
    let approve_id = open["items"][1]["id"].as_i64().unwrap();

    let approved = call_json(
        app.clone(),
        "POST",
        &format!("/api/inbox/items/{approve_id}/decision"),
        json!({"status":"approved"}),
    )
    .await;
    assert_eq!(approved.status(), StatusCode::OK);
    assert_eq!(json_body(approved).await["item"]["status"], "approved");
    let chosen = call_json(
        app.clone(),
        "POST",
        &format!("/api/inbox/items/{choose_id}/decision"),
        json!({"status":"chosen","choice":"two"}),
    )
    .await;
    ...
    let decided = json_body(
        call(
            app.clone(),
            "GET",
            "/api/inbox/items?source=sender&status=decided&applyState=pending",
        )
        .await,
    )
    .await;
    ...
    for id in [approve_id, choose_id] {
        let result = call_json(
            app.clone(),
            "POST",
            &format!("/api/inbox/items/{id}/apply-result"),
            json!({"state":"applied","resultPath":"opaque/result.md"}),
        )
        .await;
        assert_eq!(result.status(), StatusCode::OK);
        assert_eq!(json_body(result).await["item"]["applyState"], "applied");
    }
}
...
async fn inbox_alert_resend_reopens_and_http_errors_follow_contract() {
    ...
    let alert = inbox_batch(
        ...
        "today",
        json!([
            {"slug":"alert","kind":"alert","title":"watch this"}
        ]),
    );
    let saved = call_json(app.clone(), "POST", "/api/inbox/batches", alert.clone()).await;
    let id = json_body(saved).await;
    assert_eq!(id["itemCount"], 1);
    let open = json_body(call(app.clone(), "GET", "/api/inbox/items?status=open").await).await;
    let id = open["items"][0]["id"].as_i64().unwrap();
    let acknowledged = call_json(
        app.clone(),
        "POST",
        &format!("/api/inbox/items/{id}/decision"),
        json!({"status":"acknowledged"}),
    )
    .await;
    assert_eq!(acknowledged.status(), StatusCode::OK);
    assert_eq!(
        json_body(acknowledged).await["item"]["status"],
        "acknowledged"
    );
    let resent = call_json(app.clone(), "POST", "/api/inbox/batches", alert).await;
    assert_eq!(resent.status(), StatusCode::OK);
    let open = json_body(call(app.clone(), "GET", "/api/inbox/items?status=open").await).await;
    assert_eq!(open["items"][0]["status"], "open");

    let invalid = call_json(app.clone(), "POST", "/api/inbox/batches", json!({"source":"bad","date":"today","items":[{"slug":"x","kind":"approve","title":"x","options":[]}]})).await;
    assert_eq!(invalid.status(), StatusCode::BAD_REQUEST);
    assert_eq!(json_body(invalid).await["error"]["code"], "bad_request");
    let missing = call_json(
        app.clone(),
        "POST",
        "/api/inbox/items/99999/decision",
        json!({"status":"approved"}),
    )
    .await;
    assert_eq!(missing.status(), StatusCode::NOT_FOUND);
    assert_eq!(json_body(missing).await["error"]["code"], "not_found");
    let protected = inbox_batch(
        "protected",
        "2026-07-25",
        json!([
            {"slug":"approve","kind":"approve","title":"approve"}
        ]),
    );
    let first = call_json(app.clone(), "POST", "/api/inbox/batches", protected.clone()).await;
    assert_eq!(first.status(), StatusCode::OK);
    let open = json_body(call(app.clone(), "GET", "/api/inbox/items?status=open").await).await;
    let protected_id = open["items"]
        .as_array()
        .unwrap()
        .iter()
        .find(|item| item["source"] == "protected")
        .unwrap()["id"]
        .as_i64()
        .unwrap();
    assert_eq!(
        call_json(
            app.clone(),
            "POST",
            &format!("/api/inbox/items/{protected_id}/decision"),
            json!({"status":"rejected"})
        )
        .await
        .status(),
    ...
    let conflict = call_json(app, "POST", "/api/inbox/batches", protected).await;
    assert_eq!(conflict.status(), StatusCode::CONFLICT);
    assert_eq!(json_body(conflict).await["error"]["code"], "conflict");
}
